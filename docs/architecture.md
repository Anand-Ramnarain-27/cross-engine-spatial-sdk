...
## Standalone Viewer (Phase 9 — first complete demonstration)

`examples/StandaloneViewer` is the SDK's first end-to-end application: a
real Win32 window, `D3D11Renderer` (the first concrete `IRenderer` — see
[rendering.md](rendering.md) for the HLSL/`Mat4` convention-matching detail
that made this non-trivial), a free-fly camera, and a per-frame loop that
wires `StreamingManager`, `LODManager<TileId>`, `GPUUploadQueue`, and
`DebugRenderer` together against a dataset generated by `SpatialTileBuilder`.

This is also the orchestration code the architecture has been anticipating
since Phase 6: `main.cpp` owns a `TileGPU` record per resident tile (its
uploaded meshes for *every* LOD, since a tile file already contains them
all — see [lod.md](lod.md)); `StreamingManager` says which tiles are
resident, `LODManager` says which already-uploaded LOD to draw each frame,
`GPUUploadQueue` bounds how many uploads happen per frame, and
`DebugRenderer` draws every in-radius tile's bounds color-coded by
`StreamingManager::stateOf()`. No SDK module depends on another one it
shouldn't — this file is where composition happens, exactly as designed.

One implementation subtlety worth recording: upload callbacks capture a
reference to their tile's `TileGPU` entry, but a tile can be evicted from
`StreamingManager` before an upload queued for it finishes (a real race,
not a hypothetical one, once tiles move faster than the upload budget can
drain). `main.cpp` defers actually erasing a `TileGPU` entry until every
upload that captured a reference to it has completed, rather than erasing
as soon as the tile stops being desired — the alternative is a genuine
use-after-free.
...
end-to-end via `--run-seconds` automated smoke testing (window/renderer
init succeeds, tiles stream and become resident with zero load failures,
clean exit) *and* real human visual confirmation on screen — shaded
ground/building geometry and the color-coded debug wireframe both looked
correct. The latter is what caught a real bug: the debug wireframe
originally used `TileIndex`'s bounds, whose Y range is
`DatasetManifest::worldBounds()`'s generic `[-1000, 1000]` placeholder (no
real per-dataset height field exists yet), making every wireframe box 2000
units tall instead of hugging its tile — visible on screen as long vertical
spikes, invisible to any automated test that never renders a frame. Fixed
by preferring a resident tile's own bounds() (tight to its actual content)
for the debug draw.

## Status

This document will be extended as each phase lands. Current state: Phase 9
(standalone viewer, D3D11 backend, full Streaming+LOD+Rendering+Debug
wiring) complete, on top of Phases 4–8. 143 automated SDK tests (the viewer
itself is Win32/GPU-dependent and isn't part of that suite — see
